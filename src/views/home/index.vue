...
export default {
  ...
  data () {
    return {
      // 标签页绑定的数据
      active: 0,
      // 定义一个空，用来接收所有的标签页数据 我们获取频道的时候就是从本地存储来拿的，如果本地存储没有数据的话，那么就给channels赋值一个空数组
      channels: getItem('myChannels') || [],
  ...
  watch: {
    // 在watch中监听show的状态，如果show为false的话，那么就将isEdit也设置成false
    show () {
      if (!this.show) {
        this.isEdit = this.show
      }
    },
    // 为了达到我的频道数据持久化，我们暂时先将我的频道数据存到本地存储中
    channels () {
      // 其实watch中的数据就是监听的data中的数据，如果data中的数据发生变化的时候，watch就会执行相对应的逻辑，针对这个方法就是可以理解成每当数据成员中的channels发生变化的时候，就会被watch监听到，只要发生变化，就会执行watch中的方法
  ...
  methods: {
    ...
    async getUserChannelList () {
      // 我们定义一个空数组，用来接收本地存储获取到的我的频道
      let Channel = []
      // 从本地获取一下我的频道
      const localChannel = getItem('myChannels')
